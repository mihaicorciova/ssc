...
public class MainController implements Initializable {
	...
	private static final String SIDE_MENU_LAYOUT_FILE = "/fxml/SideMenuAlternative.fxml";
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			final MenuBar menuBar = (MenuBar) menuLoader.load(getClass()
					.getResourceAsStream(MENU_LAYOUT_FILE));
			AnchorPane.setLeftAnchor(menuBar, 0.0);
			AnchorPane.setRightAnchor(menuBar, 0.0);
			menuContainer.getChildren().add(menuBar);

			final FXMLLoader sideMenuLoader = new FXMLLoader();
			final AnchorPane sideMenu = (AnchorPane) sideMenuLoader.load(getClass()
					.getResourceAsStream(SIDE_MENU_LAYOUT_FILE));
			...
			final Label statusLabel = (Label) statusBarLoader.load(getClass()
					.getResourceAsStream(STATUS_BAR_LAYOUT_FILE));
